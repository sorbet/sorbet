...
public:
    ...
    void processBinding(ast::Context ctx, cfg::Binding &bind, int loopCount) {
        shared_ptr<Type> tp;
        typecase(bind.value.get(), [&](cfg::Ident *i) { tp = getType(i->what); },
                 [&](cfg::Send *i) { Error::notImplemented(); }, [&](cfg::New *i) { Error::notImplemented(); },
                 [&](cfg::Super *i) { Error::notImplemented(); },
                 [&](cfg::FloatLit *i) { tp = make_shared<Literal>(i->value); },
                 [&](cfg::IntLit *i) { tp = make_shared<Literal>(i->value); },
                 [&](cfg::StringLit *i) { tp = make_shared<Literal>(i->value); },
                 [&](cfg::Nil *i) { tp = Types::nil(); }, [&](cfg::Self *i) { tp = make_shared<ClassType>(i->klass); },
                 [&](cfg::NotSupported *i) {}, [&](cfg::Return *i) { tp = Types::bottom(); },
                 [&](cfg::LoadArg *i) {
