...
public:
    ...
    vector<shared_ptr<types::Type>> types;

    shared_ptr<types::Type> &getType(ast::SymbolRef symbol) {
    ...
    void processBinding(ast::Context ctx, cfg::Binding &bind, int loopCount) {
        ...
        typecase(
            ...
            [&](cfg::FloatLit *i) { tp = make_shared<types::Literal>(i->value); },
            [&](cfg::IntLit *i) { tp = make_shared<types::Literal>(i->value); },
            [&](cfg::StringLit *i) { tp = make_shared<types::Literal>(i->value); },
            [&](cfg::Nil *i) { tp = types::Types::nil(); },
            [&](cfg::Self *i) { tp = make_shared<types::ClassType>(i->klass); },
            [&](cfg::NotSupported *i) { tp = types::Types::dynamic(); },
            [&](cfg::Return *i) {
                auto expectedType = make_shared<types::ClassType>(
                    ctx.owner.info(ctx).result().orElse(ast::GlobalState::defn_dynamic()));
                if (!types::Types::isSubType(ctx, getType(i->what), expectedType)) {
        ...
        shared_ptr<types::Type> &cur = getType(bind.bind);

        if (loopCount == 0) {
            cur = tp;
        } else {
            if (!types::Types::isSubType(ctx, tp, cur)) {
...
void ruby_typer::infer::Inference::run(ast::Context ctx, cfg::CFG &cfg) {
    ...
    for (cfg::BasicBlock *bb : cfg.backwardsTopoSort) {
        ...
        for (ast::SymbolRef arg : bb->args) {
            ...
            current.types.emplace_back(types::Types::bottom());
        }
