...
void Environment::mergeWith(core::Context ctx, const Environment &other, cfg::CFG &inWhat, cfg::BasicBlock *bb,
                            KnowledgeFilter &knowledgeFilter) {
    // TODO(jez) This is currently true, but we will need to relax this in order to implement this change.
    // ENFORCE(!other.isDead);
    // That means that this line is currently a no-op, which is good because we'll need to delete
    // this in service of this "merging dead environments" change, so it's good to know that this
    // line is currently not load bearing.
