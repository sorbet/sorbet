...
module T::Props::Utils
  ...
  def self.need_nil_read_check?(prop_rules)
    ...
    prop_rules[:optional].nil? || prop_rules[:optional] == :on_load || prop_rules[:notify_on_nil_write]
  end
  ...
  def self.need_nil_write_check?(prop_rules)
    # . optional modifier can take: true/false/:on_load/:existing/nil
    # . except {optional: true} and {optional: :existing}, all other modes disallow nil writes
