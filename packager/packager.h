...
public:
    ...
    // Remove either the test or non-test exports from the package definition in `file`, as we process the package
    // definition with both the application and test sources, which will occur in different strata.
    //
    // NOTE: this can be removed if we ever switch to keeping tests in a completely separate package.
    static ast::ParsedFile copyPackageWithoutExports(const core::GlobalState &gs, const ast::ParsedFile &file,
