...
class PackageDBProvider : public Provider {
    core::packages::PackageDB &packageDB;

public:
    PackageDBProvider(core::packages::PackageDB &packageDB) : packageDB(packageDB) {}
...
void packageRunCore(core::GlobalState &gs, WorkerPool &workers, absl::Span<ast::ParsedFile> files) {
...

    {
        ...
        if constexpr (buildPackageDB) {
            if (gs.packageDB().enforceLayering()) {
                PackageDBProvider provider{gs.packageDB()};
                gs.packageDB().setCondensation(ComputePackageSCCs::run(gs, provider));
            }
        }
