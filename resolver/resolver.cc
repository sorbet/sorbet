...
private:
    ...
    static PackageResolutionAction packageResolutionAction(core::Context ctx, const ConstantResolutionItem &job,
        ...
        if (!alreadyReported && !productionLegacyTestPath && inaccessibleCursorPackage) {
            auto cursorIsAvailablePrefix = isStrictPrefixOfAvailablePackage(ctx, cursorPackage);
            bool shouldReport;
            switch (job.packageCursorPosition) {
                case PackageCursorPosition::PackageBoundary:
                    shouldReport = job.isOutermost || !cursorIsAvailablePrefix;
                    break;
    ...
    static void constantResolutionFailed(core::GlobalState &gs, core::FileRef file, ConstantResolutionItem &job,
        ...
        if (shouldCheckPackage(ctx) && !job.isOutermost && job.packageRegistryCursor.exists() &&
            ...
            // Runtime resolution can fail at an intermediate package namespace before package-directed traversal has
            // loaded that namespace. Preserve the package-registry cursor so the outer component can keep resolving.
        ...
        if (packageAction == PackageResolutionAction::ReportPackageError) {
            auto cursorPackage = core::packages::MangledName(job.packageRegistryCursor);
        ...
        if (packageAction != PackageResolutionAction::None) {
            silenceError = true;
        }
