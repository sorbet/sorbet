...
parser::Node *signaturesTargetPrism(parser::Node *node) {
...
CommentsPrism SigsRewriterPrism::commentsForNode(parser::Node *node) {
    ...
    if (legacyCommentsByNode != nullptr) {
        if (auto commentsNodesEntry = legacyCommentsByNode->find(node); commentsNodesEntry != legacyCommentsByNode->end()) {
...
unique_ptr<parser::NodeVec> SigsRewriterPrism::signaturesForNode(parser::Node *node) {
    auto comments = commentsForNode(node);
...
parser::NodeVec SigsRewriterPrism::rewriteNodes(parser::NodeVec nodes) {
...
unique_ptr<parser::Node> SigsRewriterPrism::rewriteBegin(unique_ptr<parser::Node> node) {
    ...
    for (auto &stmt : oldStmts) {
        if (auto target = signaturesTargetPrism(stmt.get())) {
...
unique_ptr<parser::Node> SigsRewriterPrism::rewriteBody(unique_ptr<parser::Node> node) {
    if (node == nullptr) {
        return node;
    }
    ...
    if (auto target = signaturesTargetPrism(node.get())) {
        if (auto signatures = signaturesForNode(target)) {
...
unique_ptr<parser::Node> SigsRewriterPrism::rewriteClass(unique_ptr<parser::Node> node) {
    if (node == nullptr) {
        return node;
    }

    auto comments = commentsForNode(node.get());
    auto helpers = extractHelpersPrism(ctx, comments.annotations);
    ...
    typecase(
        node.get(),
        [&](parser::Class *klass) {
            klass->body = maybeWrapBodyPrism(node, move(klass->body));
            if (!helpers.empty()) {
                maybeInsertExtendTHelpersPrism(&klass->body);
                insertHelpersPrism(&klass->body, move(helpers));
            }
            insertTypeParams(klass, &klass->body);
        },
        [&](parser::Module *module) {
            module->body = maybeWrapBodyPrism(node, move(module->body));
            if (!helpers.empty()) {
                maybeInsertExtendTHelpersPrism(&module->body);
                insertHelpersPrism(&module->body, move(helpers));
            }
            insertTypeParams(module, &module->body);
        },
        [&](parser::SClass *sclass) {
            sclass->body = maybeWrapBodyPrism(node, move(sclass->body));
            if (!helpers.empty()) {
                maybeInsertExtendTHelpersPrism(&sclass->body);
                insertHelpersPrism(&sclass->body, move(helpers));
            }
...
unique_ptr<parser::Node> SigsRewriterPrism::rewriteNode(unique_ptr<parser::Node> node) {
    if (node == nullptr) {
        return node;
    }
