...
pair<optional<MethodType>, optional<ParseError>> RBSParser::parseSignature(core::Context ctx, Comment comment) {
    ...
    auto parser = std::shared_ptr<parserstate>(alloc_parser(rbsString, encoding, 0, comment.string.size()),
    ...
    return make_pair(MethodType{comment.loc, rbsMethodType, parser}, nullopt);
}
...
pair<optional<Type>, optional<ParseError>> RBSParser::parseType(core::Context ctx, Comment comment) {
    ...
    return make_pair(Type{comment.loc, rbsType, parser}, nullopt);
}
