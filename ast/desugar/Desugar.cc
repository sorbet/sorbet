...
unique_ptr<Expression> stat2Expr(unique_ptr<Statement> &expr) {
    Error::check(dynamic_cast<Expression *>(expr.get()));
    return unique_ptr<Expression>(dynamic_cast<Expression *>(expr.release()));
}
...
unique_ptr<Statement> mkIf(unique_ptr<Statement> &cond, unique_ptr<Statement> &thenp, unique_ptr<Statement> &elsep) {
    return make_unique<If>(stat2Expr(cond), stat2Expr(thenp), stat2Expr(elsep));
}
    ...
    typecase(what.get(),
             [&](parser::And *a) {
                 ...
                 if (auto i = dynamic_cast<Ident *>(lhs.get())) {
                     ...
                     auto wrapped = mkInsSeq1(move(temp), stat2Expr(iff));
             ...
             [&](parser::Const *a) {
                 ...
                 unique_ptr<Statement> res = make_unique<ConstantLit>(stat2Expr(scope), a->name);
                 result.swap(res);
             },
             [&](parser::Begin *a) {
                 vector<unique_ptr<Statement>> stats;
                 ...
                 if (auto *epx = dynamic_cast<Expression *>(expr.get())) {
                     ...
                     auto block = mkInsSeq(move(stats), stat2Expr(mkEmptyTree()));
                     result.swap(block);
                 }
             },
             ...
             [&](parser::Block *block) {
                 ...
                 unique_ptr<Statement> res =
                     make_unique<Block>(move(send), args, stat2Expr(node2TreeImpl(ctx, block->body)));
                 result.swap(res);
             },
             ...
             [&](parser::LVar *var) {
                 unique_ptr<Statement> res = make_unique<Ident>(var->name, ContextBase::defn_lvar_todo());
                 result.swap(res);
             },
             [&](parser::GVar *var) {
                 unique_ptr<Statement> res = make_unique<Ident>(var->name, ContextBase::defn_gvar_todo());
                 result.swap(res);
             },
             [&](parser::CVar *var) {
             ...
             [&](parser::LVarAsgn *asgn) {
                 unique_ptr<Statement> lhs = make_unique<Ident>(asgn->name, ContextBase::defn_lvar_todo());
                 auto rhs = node2TreeImpl(ctx, asgn->expr);
                 auto res = mkAssign(lhs, rhs);
                 result.swap(res);
             },
             [&](parser::GVarAsgn *asgn) {
                 unique_ptr<Statement> lhs = make_unique<Ident>(asgn->name, ContextBase::defn_gvar_todo());
                 auto rhs = node2TreeImpl(ctx, asgn->expr);
                 auto res = mkAssign(lhs, rhs);
                 result.swap(res);
             },
             [&](parser::CVarAsgn *asgn) {
                 unique_ptr<Statement> lhs = make_unique<Ident>(asgn->name, ContextBase::defn_cvar_todo());
                 auto rhs = node2TreeImpl(ctx, asgn->expr);
                 auto res = mkAssign(lhs, rhs);
                 result.swap(res);
             },
             [&](parser::ConstAsgn *constAsgn) {
                 ...
                 unique_ptr<Statement> lhs = make_unique<ConstantLit>(stat2Expr(scope), constAsgn->name);
