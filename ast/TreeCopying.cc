...
ExpressionPtr deepCopy(const void *avoid, const Tag tag, const void *tree, bool root) {
    ...
    switch (tag) {
        ...
        case Tag::UnresolvedConstantLit: {
            ...
            return make_expression<UnresolvedConstantLit>(exp->loc, deepCopy(avoid, exp->rootScope_), exp->segments_);
        }
        ...
        case Tag::ConstantLit: {
            ...
            if (auto *original = exp->original()) {
                originalC = make_unique<UnresolvedConstantLit>(original->loc, deepCopy(avoid, original->rootScope_),
                                                               original->segments_);
