...
void createInitialGlobalState(unique_ptr<core::GlobalState> &gs, const realmain::options::Options &options,
    ...
    if (options.noStdlib) {
        ...
        ENFORCE(
            ...
            gs->utf8NamesUsed(), core::GlobalState::PAYLOAD_MAX_UTF8_NAME_COUNT);
        ENFORCE(gs->constantNamesUsed() < core::GlobalState::PAYLOAD_MAX_CONSTANT_NAME_COUNT,
                "Payload defined `{}` Constant names, which is greater than the expected maximum of `{}`. Consider "
                "updating "
        ...
        ENFORCE(
            ...
            gs->typeMembersUsed(), core::GlobalState::PAYLOAD_MAX_TYPE_MEMBER_COUNT);
        ENFORCE(gs->typeArgumentsUsed() < core::GlobalState::PAYLOAD_MAX_TYPE_ARGUMENT_COUNT,
                "Payload defined `{}` type arguments, which is greater than the expected maximum of `{}`. Consider "
                "updating "
