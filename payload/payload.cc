...
void createInitialGlobalState(core::GlobalState &gs, const realmain::options::Options &options,
    ...
    if (kvstore) {
        ...
        auto maybeDiffCountAndHashSizeBytes =
        ...
        if (maybeUUIDBytes.data != nullptr) {
            ...
            if (maybeDiffCountAndHashSizeBytes.data != nullptr) {
                auto diffCount = core::serialize::Serializer::loadNameTableDiffCountAndResizeNamesHash(
                    gs, maybeDiffCountAndHashSizeBytes.data);
                ENFORCE(diffCount > 0);
                ...
                // TODO(neil): In the future, we could compact diffs to avoid reading many small entries.
                ...
                gs.setNameTableDiffCount(diffCount);
                gs.markNameTableAsCached();
                // Check that well-known names are consistent with what the payload would expect.
                if constexpr (debug_mode) {
                    // The name of the Sorbet class is well-known, and the name is generated by
                    // core/tools/generate_names.cc. If this doesn't match up, something has gone seriously wrong
                    // with name table serialization.
                    ...
                    // Array#bsearch is defined in the payload rbis with no overloads. If that changes, pick a new
                    // method that's defined in an rbi, but on a well-known class.
