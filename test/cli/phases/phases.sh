...
for p in parse-tree parse-tree-json ast ast-raw dsl-tree dsl-tree-raw name-table name-tree name-tree-raw resolve-tree resolve-tree-raw file-table-json cfg cfg-raw typed-source; do
    echo "--- $p start ---"
    main/sorbet -p "$p" -e '1'
    echo "--- $p end ---"
done
