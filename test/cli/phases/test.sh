...
for p in parse-tree parse-tree-json desugar-tree desugar-tree-raw local-vars-tree local-vars-tree-raw rewrite-tree rewrite-tree-raw index-tree index-tree-raw name-tree name-tree-raw resolve-tree resolve-tree-raw flatten-tree flatten-tree-raw ast ast-raw cfg cfg-raw cfg-text symbol-table symbol-table-raw; do
    echo "--- $p start ---"
...
for p in init parser desugarer rewriter local-vars namer resolver cfg inferencer; do
