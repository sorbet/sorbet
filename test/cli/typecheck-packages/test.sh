...
run() {
  ...
  output=$("$sorbet" --silence-dev-message --censor-for-snapshot-tests --max-threads=0 \
    --no-error-sections --no-error-count "$@" . 2>&1)
  status=$?
...
echo '--- Target: dependencies, consumers, their dependencies, and tests ---'
run --stripe-packages --typecheck-packages=Target

echo '--- Same selection in package-directed mode ---'
run --stripe-packages --experimental-package-directed --typecheck-packages=Target
