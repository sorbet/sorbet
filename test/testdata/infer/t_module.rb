# typed: true
extend T::Sig

sig { params(x: T.all(T::Props, Kernel)).void }
def example(x)
  klass = x.class
  T.reveal_type(klass) # error: `T.untyped`
end

sig { params(mod: Module).void }
def example2(mod)
  if mod < T::Props
    T.reveal_type(mod) # error: `T::Module[T.anything]`

    if mod.is_a?(Class)
      T.reveal_type(mod) # error: `T::Class[T.anything]`
      inst = mod.new
      T.reveal_type(inst) # error: `T.anything`
    end
  end

  if mod < T::Props::Serializable && mod.is_a?(T::Props::Serializable::ClassMethods)
    T.reveal_type(mod) # error: `T.all(T::Module[T.anything], T::Props::Serializable::ClassMethods)`

    if mod.is_a?(Class)
      T.reveal_type(mod) # error: `T.all(T::Class[T.anything], T::Props::Serializable::ClassMethods)`
      inst = mod.new
      T.reveal_type(inst) # error: `T.anything`
    end
  end
end

class NoAttachedClassRequired < Module
  # this file has a typed level of true, so has_attached_class! is not required
end
