...
def int_map(&blk)
  ...
  ["a", "b"].map(&blk) # error: Expected `T.proc.params(arg0: String).returns(T.unknown)` but found `T.proc.params(arg0: Integer).returns(String)` for block argument
end

sig {params(blk: Proc).void}
def unknown_arity(&blk)
  a = [1, 2, 3].map(&blk) # error: Cannot use a `Proc` with unknown arity as a `T.proc.params(arg0: Integer).returns(T.unknown)`
