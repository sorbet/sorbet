...
// optional<unique_ptr<SorbetTypecheckRunInfo>> getTypecheckRunInfo(LSPMessage &msg) {
    // if (msg.isNotification() && msg.method() == LSPMethod::SorbetTypecheckRunInfo) {
        // unique_ptr<SorbetTypecheckRunInfo> &typecheckInfo = get<unique_ptr<SorbetTypecheckRunInfo>>(msg.asNotification().params);
...
// TEST_CASE_FIXTURE(MultithreadedProtocolTest, "CancelsSlowPathWhenNewEditWouldTakeSlowPath") {
    ...
    // {
        ...
        // if (message->isNotification() && message->method() == LSPMethod::TextDocumentPublishDiagnostics) {
            // message = readAsync();
        // }
        // auto status = getTypecheckRunStatus(*message);
        // REQUIRE(status.has_value());
...
// TEST_CASE_FIXTURE(MultithreadedProtocolTest, "CanPreemptSlowPathWithFastPathAndBothErrorsAreReported") {
    ...
    // stopInDebugger();
    // auto resp = send(LSPMessage(make_unique<NotificationMessage>("2.0", LSPMethod::SorbetFence, 20)));
    // for (const auto &m : resp) {
        // fmt::print("*** readAsync: {}\n", m->toJSON(true));
    // }
    // assertErrorDiagnostics(move(resp),
                           // {
