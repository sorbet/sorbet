...
class SorbetRBIGeneration::RbiGenerator
  ...
  def self.gem_from_location(location)
    match =
  ...
  def self.class_name(klass)
    ...
    if name.nil? || name == ""
      ...
      id = @anonymous_map[real_object_id(klass)] ||= anonymous_id
  ...
  def self.generate_method(method, instance, spaces = 2)
