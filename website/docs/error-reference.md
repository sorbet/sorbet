...
This error occurs in `--experimental-test-packages` mode, when an invalid import was detected. There are two different scenarios that will produce this error, let's consider the following set of packages:

- `A` - An application package that imports one constant `A::C1` and keeps `A::C2` private
...
1. If `B` imports `A` with the `uses_internals: true` annotation, as only `test!` packages are allowed to import with `uses_internals: true`, or
