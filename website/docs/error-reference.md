...
A package references constants from other package without according `import` or
`test_import` statements.

To fix this error `import` or `test_import` related packages in the
`__package.rb` file where the error is reported.
