...
**Note**: The arity of a method does not include any types the two methods
signatures' declare. For example:
...
The most commonly preferred alternative solution is to mark the original method
`private` and define a new method with a new name, instead of redefining the old
method. (Or alternatively, use Sorbet's [editor integration](vscode.md) to
rename the old method, declare that method `private`, and define a new method
...
`type_member` and `type_template` cannot be used at the top-level of a
file—instead they must be used inside a class or module definition.
