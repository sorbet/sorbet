...
would error at runtime, because `sorbet-runtime` uses metaprogramming to
remove the `Foo#foo` method. Any mistaken calls to this method would raise an
error at runtime.
...
definition, `Foo` will not exist at Runtime, and the program will error
when referencing the `Foo` class.
...
type checking, but it will not exist at runtime, resulting in obvious errors
if any child classes attempt to call it.
