...
It can be enabled by adding `sorbet.highlightUntypedValues: true` to your
VSCode's `settings.json`, or by using the
`Sorbet: Toggle Highlight untyped values` command from the command palette (note
the latter causes a full restart of Sorbet).
