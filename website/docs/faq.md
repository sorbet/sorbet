...
The `sorbet-static` gem is only tested on macOS 15.0 (Sequoia) on Apple Silicon, and Ubuntu 20.04 (Focal Fossa). We expect `sorbet-static` to work as far back as macOS 10.10 (Yosemite) and on most Linux distributions using `glibc`.

There is currently no Windows support. Supporting Windows would be a substantial amount of complexity because of how much Sorbet would be affected by differences in the filesystem & filenames, threading, subprocesses, virtual memory, pointer layouts, etc. leading to a higher maintenance burden. The predominant codebases and companies using Sorbet don't need Windows support, making the tradeoff of higher maintenance a high hurdle to clear. We are open to discussing patches that add Windows build support, but the bar would be pretty high for accepting them and we might not commit to keeping windows builds evergreen in CI, the same we do for other platforms. Anyone who is interested in contributing Windows support should follow our guide for [Contributing to Sorbet](https://github.com/sorbet/sorbet/blob/master/CONTRIBUTING.md) to engage with members of the Sorbet team.
