...
pushd tmp/bench &>/dev/null
    (time for _ in {1..10}; do ../../bazel-bin/external/sorbet_ruby/toolchain/bin/ruby -r ../../run/tools/preamble.rb -e 1 --disable=gems --disable=did_you_mean ; done) 2>&1|grep real | cut -d$'\t' -f 2 > baseline_time
...
for this_src in "${rb_src[@]}"; do
    ...
    (time for _ in {1..10}; do ../../bazel-bin/external/sorbet_ruby/toolchain/bin/ruby -r ../../run/tools/preamble.rb ./target.rb --disable=gems --disable=did_you_mean ; done) 2>&1|grep real | cut -d$'\t' -f 2 > ruby_runtime
    ...
    (time for _ in {1..10}; do ../../bazel-bin/external/sorbet_ruby/toolchain/bin/ruby -r ../../run/tools/preamble.rb -e "require './target.rb.so'" --disable=gems --disable=did_you_mean ; done) 2>&1|grep real | cut -d$'\t' -f 2 > compiled_runtime
