        ...
        for pass in "${passes[@]}" ; do
            ...
            if [ -e "$candidate" ]; then
                if [ "$pass" = "document-symbols" ]; then
                    ...
                    echo bazel-bin/main/sorbet --silence-dev-message  --suppress-non-critical --censor-for-symbol-table-exp --print "$pass" --max-threads 0 "${args[@]}" "${srcs[@]}" \> "$candidate" 2\>/dev/null >> "$COMMAND_FILE"
                fi
            fi
        done
    fi
