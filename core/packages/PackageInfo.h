...
public:
    ...
    // Track that this package is missing an import of type `importType` for `toImport`, along with an autocorrect that
    // adds the import to this package's __package.rb We use this to de-duplicate imports added by autocorrects;
    // multiple calls ot trackMissingImport for the same un-imported package will still lead to only the package being
    // imported once in the autocorrect generated by aggregateMissingImports
    ...
    // Return the autocorrect that adds an import of type `importType` to `toImport`, if it has been previously stored
    // using trackMissingImport
