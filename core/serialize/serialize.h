...
public:
    ...
    static constexpr std::string_view NAME_TABLE_HASH_SIZE_KEY = "NameTableHashSize";
    ...
    // Serialize only the name table from a global state. This is sufficient for deserializing trees that have only been
    ...
    static void loadAndResizeNamesHash(GlobalState &gs, const uint8_t *const data);
    static std::vector<uint8_t> storeDiffCount(uint32_t count);
    static std::vector<uint8_t> storeHashSize(const GlobalState &gs);
