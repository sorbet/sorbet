...
void SerializerImpl::pickle(Pickler &p, const ast::ExpressionPtr &what) {
    ...
    switch (tag) {
        ...
        case ast::Tag::NonEmptyArray: {
            auto &a = ast::cast_tree_nonnull<ast::NonEmptyArray>(what);
            pickle(p, a.loc);
            p.putU4(a.elems.size());
            for (auto &e : a.elems) {
                pickle(p, e);
...
ast::ExpressionPtr SerializerImpl::unpickleExpr(serialize::UnPickler &p, const GlobalState &gs) {
    ...
    switch (static_cast<ast::Tag>(kind)) {
        ...
        case ast::Tag::NonEmptyArray: {
            auto loc = unpickleLocOffsets(p);
            auto sz = p.getU4();
            ast::NonEmptyArray::ENTRY_store elems(sz);
            for (auto &elem : elems) {
                elem = unpickleExpr(p, gs);
            }
            return ast::MK::NonEmptyArray(loc, std::move(elems));
        }
        case ast::Tag::Cast: {
            auto loc = unpickleLocOffsets(p);
