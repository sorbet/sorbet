...
vector<uint8_t> Serializer::storeHashSize(const GlobalState &gs) {
    Pickler p;
    p.putU4(Serializer::VERSION);
    p.putU4(gs.namesByHash.size());
    return p.result();
...
void Serializer::loadAndResizeNamesHash(GlobalState &gs, const uint8_t *const data) {
    gs.utf8Names.clear();
    ...
    auto namesByHashSize = p.getU4();
    if (namesByHashSize > gs.namesByHash.size()) {
