...
string ShapeType::show(const GlobalState &gs) const {
    ...
    for (auto &key : this->keys) {
        ...
        SymbolRef undSymbol =
            cast_inline_type_nonnull<ClassType>(cast_inline_type_nonnull<LiteralType>(key).underlying()).symbol;
        if (undSymbol == Symbols::Symbol()) {
            fmt::format_to(buf, "{}: {}", NameRef(gs, cast_inline_type_nonnull<LiteralType>(key).value).show(gs),
