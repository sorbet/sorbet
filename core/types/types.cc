...
DispatchArgs DispatchArgs::withSelfAndThisRef(const TypePtr &newSelfRef) const {
    return DispatchArgs{
        ...
        isPrivateOk, suppressErrors, core::NameRef()};
}
...
DispatchArgs DispatchArgs::withErrorsSuppressed() const {
    return DispatchArgs{
        name, locs, numPosArgs, args, selfType, fullType, thisType, block, originForUninitialized, isPrivateOk, true, core::NameRef()};
}
