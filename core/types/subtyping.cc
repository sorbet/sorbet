...
TypePtr Types::lub(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2) {
    ...
    if (isa_type<ClassType>(t1)) {
        auto mayBeSpecial1 = cast_inline_type_nonnull<ClassType>(t1);
    ...
    if (isa_type<ClassType>(t2)) {
        auto mayBeSpecial2 = cast_inline_type_nonnull<ClassType>(t2);
    ...
    if (is_proxy_type(t1)) {
        ...
        if (is_proxy_type(t2)) {
                ...
                if (auto *h2 = cast_type_const<ShapeType>(t2)) {
                    if (h2->keys.size() == h1->keys.size()) {
                        ...
                        for (auto &el2 : h2->keys) {
                            ++i;
                            auto el2l = cast_inline_type_nonnull<LiteralType>(el2);
                            auto u2 = cast_inline_type_nonnull<ClassType>(el2l.underlying());
                            auto fnd = absl::c_find_if(h1->keys, [&](auto &candidate) -> bool {
                                auto el1l = cast_inline_type_nonnull<LiteralType>(candidate);
                                auto u1 = cast_inline_type_nonnull<ClassType>(el1l.underlying());
            ...
            } else if (isa_type<LiteralType>(t1)) {
                if (isa_type<LiteralType>(t2)) {
                    auto l1 = cast_inline_type_nonnull<LiteralType>(t1);
                    auto l2 = cast_inline_type_nonnull<LiteralType>(t2);
                    auto u1 = cast_inline_type_nonnull<ClassType>(l1.underlying());
                    auto u2 = cast_inline_type_nonnull<ClassType>(l2.underlying());
    ...
    {
        if (isa_type<SelfTypeParam>(t1) || isa_type<SelfTypeParam>(t2)) {
            // NOTE: SelfTypeParam is an inlined type, so TypePtr equivalence is type equivalence.
            if (t1 != t2) {
                return OrType::make_shared(t1, t2);
            } else {
                return t1;
...
TypePtr lubGround(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2) {
    ...
    // 1 :-)
    auto c1 = cast_inline_type_nonnull<ClassType>(t1);
    auto c2 = cast_inline_type_nonnull<ClassType>(t2);
...
TypePtr Types::glb(const GlobalState &gs, const TypePtr &t1, const TypePtr &t2) {
    ...
    {
        if (isa_type<SelfTypeParam>(t1) || isa_type<SelfTypeParam>(t2)) {
            // NOTE: SelfTypeParam is an inline type so TypePtr equivalence is type equivalence.
            if (t1 != t2) {
                return AndType::make_shared(t1, t2);
            } else {
                return t1;
...
bool isSubTypeUnderConstraintSingle(const GlobalState &gs, TypeConstraint &constr, UntypedMode mode, const TypePtr &t1,
    ...
    {
        auto t1IsSelf = isa_type<SelfTypeParam>(t1);
        auto t2IsSelf = isa_type<SelfTypeParam>(t2);
        if (t1IsSelf || t2IsSelf) {
            // NOTE: SelfTypeParam is used both with LambdaParam and TypeVar, so
            // we can only check bounds when a LambdaParam is present.
            if (!t1IsSelf) {
                auto self2 = cast_inline_type_nonnull<SelfTypeParam>(t2);
                ...
                auto self1 = cast_inline_type_nonnull<SelfTypeParam>(t1);
    ...
    if (is_proxy_type(t1)) {
        if (is_proxy_type(t2)) {
            ...
            if (auto *a1 = cast_type_const<TupleType>(t1)) { // Warning: this implements COVARIANT arrays
                ...
                auto u1 = cast_inline_type_nonnull<ClassType>(l1.underlying());
                auto u2 = cast_inline_type_nonnull<ClassType>(l2.underlying());
                return u1.symbol == u2.symbol && l1.value == l2.value;
            } else if (auto *m1 = cast_type_const<MetaType>(t1)) {
                auto *m2 = cast_type_const<MetaType>(t2);
                if (m2 == nullptr) {
                    // is a literal a subtype of a different kind of proxy
        ...
        if (isa_type<ClassType>(t1) && isa_type<ClassType>(t2)) {
            auto c1 = cast_inline_type_nonnull<ClassType>(t1);
            auto c2 = cast_inline_type_nonnull<ClassType>(t2);
