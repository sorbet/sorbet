...
unique_ptr<Error> missingKwarg(const GlobalState &gs, Loc argsLoc, MethodRef method, const ParamInfo &arg,
    ...
    if (hasKwparams) {
        ...
        if (auto hash = cast_type<ShapeType>(kwargs)) {
            ...
            while (kwit != methodData->parameters.end()) {
                ...
                if (arg == hash->keys.end()) {
                    if (!kwParam.flags.isDefault) {
                        if (auto e = missingKwarg(gs, args.argsLoc(gs), method, kwParam, symbol, targs)) {
                            result.main.errors.emplace_back(std::move(e));
                        }
                    }
                    continue;
            ...
            for (auto &param : methodData->parameters) {
                ...
                if (auto e = missingKwarg(gs, args.argsLoc(gs), method, param, symbol, targs)) {
                    result.main.errors.emplace_back(std::move(e));
                }
            }
        }
