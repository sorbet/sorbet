...
TypePtr unwrapType(const GlobalState &gs, Loc loc, const TypePtr &tp) {
    ...
    if (auto *classType = cast_type<ClassType>(tp.get())) {
        ...
        SymbolRef attachedClass = classType->symbol.data(gs)->attachedClass(gs, classType->symbol);
        ...
        return attachedClass.data(gs)->externalType(gs, attachedClass);
    }

    if (auto *appType = cast_type<AppliedType>(tp.get())) {
        SymbolRef attachedClass = appType->klass.data(gs)->attachedClass(gs, appType->klass);
        if (!attachedClass.exists()) {
            if (auto e = gs.beginError(loc, errors::Infer::BareTypeUsage)) {
                e.setHeader("Unsupported usage of bare type");
            }
...
bool extendsTHelpers(const GlobalState &gs, core::SymbolRef enclosingClass) {
    ...
    auto enclosingSingletonClass = enclosingClass.data(gs)->lookupSingletonClass(gs, enclosingClass);
...
optional<core::AutocorrectSuggestion> maybeSuggestExtendTHelpers(const GlobalState &gs, const Type *thisType,
    ...
    auto enclosingClass = classType->symbol.data(gs)->topAttachedClass(gs, classType->symbol);
    ...
    if (!mayBeOverloaded.exists()) {
        ...
        if (e) {
            ...
            if (args.fullType.get() != thisType && symbol == Symbols::NilClass()) {
                ...
                auto alternatives = symbol.data(gs)->findMemberFuzzyMatch(gs, symbol, args.name);
                ...
                auto attached = symbol.data(gs)->attachedClass(gs, symbol);
        ...
        if (!(pit->flags.isKeyword || pit->flags.isDefault || pit->flags.isRepeated || pit->flags.isBlock)) {
            if (auto e = gs.beginError(core::Loc(args.locs.file, args.locs.call),
                ...
                if (args.name == core::Names::any() &&
                    symbol == core::Symbols::T().data(gs)->lookupSingletonClass(gs, core::Symbols::T())) {
                    e.addErrorSection(
...
public:
    void apply(const GlobalState &gs, DispatchArgs args, const Type *thisType, DispatchResult &res) const override {
        ...
        auto singleton = self.data(gs)->lookupSingletonClass(gs, self);
        if (singleton.exists()) {
            res.returnType = singleton.data(gs)->externalType(gs, singleton);
        } else {
...
public:
    void apply(const GlobalState &gs, DispatchArgs args, const Type *thisType, DispatchResult &res) const override {
        ...
        auto attachedClass = self.data(gs)->attachedClass(gs, self);
        if (!attachedClass.exists()) {
        ...
        auto instanceTy = attachedClass.data(gs)->externalType(gs, attachedClass);
...
public:
    ...
    void apply(const GlobalState &gs, DispatchArgs args, const Type *thisType, DispatchResult &res) const override {
        ...
        SymbolRef self = unwrapSymbol(thisType);
        attachedClass = self.data(gs)->attachedClass(gs, self);
...
public:
    void apply(const GlobalState &gs, DispatchArgs args, const Type *thisType, DispatchResult &res) const override {
        ...
        if (!self.data(gs)->isSingletonClass(gs)) {
            ...
            auto selfAttachedClass = self.data(gs)->attachedClass(gs, self);
            auto instanceTy = selfAttachedClass.data(gs)->externalType(gs, selfAttachedClass);
...
public:
    ...
    void apply(const GlobalState &gs, DispatchArgs args, const Type *thisType, DispatchResult &res) const override {
        auto hash = make_type<ClassType>(core::Symbols::Sorbet_Private_Static().data(gs)->lookupSingletonClass(
            gs, core::Symbols::Sorbet_Private_Static()));
...
public:
    void apply(const GlobalState &gs, DispatchArgs args, const Type *thisType, DispatchResult &res) const override {
        ...
        auto lhs = rc.data(gs)->externalType(gs, rc);
