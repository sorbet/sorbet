...
StrictLevel File::fileStrictSigil(string_view source) {
    ...
    auto sigilLoc = fileStrictSigilLocation(source);
    if (!sigilLoc.exists()) {
        return StrictLevel::None;
    }

    // sigilLoc here points to the entire comment, but we want to find
...
// This finds the location of the `# typed:` sigil in the file,
// starting from the beginning of the comment and going until the end
// of the line (but not including the newline or carriage return
// characters)
