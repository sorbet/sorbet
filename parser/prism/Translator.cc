    ...
    switch (PM_NODE_TYPE(node)) {
        ...
        case PM_DEF_NODE: { // Method definitions, like `def m; ...; end` and `def m = 123`
            ...
            if (defNode->parameters != nullptr) {
                ...
                std::tie(paramsStore, statsStore, enclosingBlockParamName) =
                ...
                if (rparenLoc.start != nullptr) {
                    ...
                    std::tie(paramsStore, statsStore, enclosingBlockParamName) = desugarParametersNode(nullptr, loc);
                }
...
Translator::desugarParametersNode(pm_parameters_node *paramsNode, core::LocOffsets location,
    ...
    if (paramsNode == nullptr) {
        ...
        return {move(paramsStore), ast::InsSeq::STATS_store{}, core::Names::blkArg()};
    }
    ...
    return make_tuple(move(paramsStore), move(statsStore), enclosingBlockParamName);
}
