...
ast::ExpressionPtr Translator::buildConstantPath(pm_node_t *parentNullable, core::LocOffsets outerLoc,
                                                  core::NameRef outerConstName,
                                                  core::LocOffsets nullParentRootLoc) {
        ...
        while (true) {
            if (PM_NODE_TYPE_P(current, PM_CONSTANT_PATH_NODE)) {
                ...
                if (pathNode->name != PM_CONSTANT_ID_UNSET) {
                    pathConstName =
                        ctx.state.enterNameConstant(translateConstantName(pathNode->name));
                } else {
