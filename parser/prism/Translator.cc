...
unique_ptr<parser::Node> Translator::translate(pm_node_t *node) {
    ...
    switch (PM_NODE_TYPE(node)) {
        ...
        case PM_DEF_NODE: { // Method definitions, like `def m; ...; end` and `def m = 123`
            ...
            std::tie(argsStore, statsStore, didDesugarParams) = desugarParametersNode(params, attemptToDesugarParams);

            if (!hasExpr(receiver, body) || !didDesugarParams) {
        ...
        case PM_OPTIONAL_KEYWORD_PARAMETER_NODE: { // An optional keyword parameter, like `def foo(a: 1)`
            ...
            auto expr = MK::OptionalArg(location, MK::KeywordArg(nameLoc, name), value->takeDesugaredExpr());
        ...
        case PM_OPTIONAL_PARAMETER_NODE: { // An optional positional parameter, like `def foo(a = 1)`
            ...
            auto expr = MK::OptionalArg(location, MK::Local(nameLoc, name), value->takeDesugaredExpr());
            return make_node_with_expr<parser::Optarg>(move(expr), location, name, nameLoc, move(value));
        }
...
Translator::desugarParametersNode(unique_ptr<parser::Args> &paramsNode, bool attemptToDesugarParams) {
    ...
    auto supportedParams = absl::c_all_of(paramsNode->args, [](auto &param) {
        return param->hasDesugaredExpr() ||
               ...
               // representation, so they won't be directly translated. Instead, they have
               // special desugar logic below.
               parser::NodeWithExpr::isa_node<parser::Kwrestarg>(param.get()) ||
               parser::NodeWithExpr::isa_node<parser::Kwnilarg>(param.get()) ||
               parser::NodeWithExpr::isa_node<parser::ForwardArg>(param.get());
    });
