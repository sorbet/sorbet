...
optional<PropInfo> parseProp(core::MutableContext ctx, const ast::Send *send) {
    ...
    switch (send->fun.rawId()) {
        ...
        case core::Names::updatedProp().rawId(): {
            ...
            // 5 is the length of the _prop suffix
            ret.nameLoc = core::LocOffsets{send->loc.beginPos(), send->loc.endPos() - 5};
            ret.type = ASTUtil::mkNilable(send->loc, ast::MK::UnresolvedConstant(ast::MK::EmptyTree(),
        ...
        case core::Names::merchantTokenProp().rawId():
            ...
            ret.type = ast::MK::UnresolvedConstantParts(
                send->loc,
                {core::Names::Constants::Opus(), core::Names::Constants::Autogen(),
                 core::Names::Constants::Tokens(), core::Names::Constants::AccountModelMerchant(),
