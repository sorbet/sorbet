...
vector<ast::ExpressionPtr> Struct::run(core::MutableContext ctx, ast::Assign *asgn) {
    ...
    structUniqueNameUnresolvedConst.mutableNames().back() =
        ctx.state.enterNameConstant(ctx.state.freshNameUnique(core::UniqueNameKind::Struct, lhs->names().back(), 1));
