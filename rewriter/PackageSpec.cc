    ...
    for (int i = (int)ns.size() - 1; i >= 0; --i) {
        ...
        if (absl::StrContains(name.shortName(ctx), "_")) {
            ...
            if (auto e = ctx.beginIndexerError(loc, core::errors::Packager::InvalidPackageName)) {
                ...
                e.addAutocorrect(
                    core::AutocorrectSuggestion{fmt::format("Replace `{}` with `{}`", name.shortName(ctx), replacement),
...
void PackageSpec::run(core::MutableContext ctx, ast::ClassDef *klass) {
    ...
    for (auto &rootStmt : klass->rhs) {
        ...
        if (superClassLit == nullptr || superClassLit->names().back() != core::Names::Constants::PackageSpec()) {
