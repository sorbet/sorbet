...
export class SorbetStatusProvider implements Disposable {
  private sorbetLanguageClient?: SorbetLanguageClient;
  ...
  public get activeSorbetLanguageClient(): SorbetLanguageClient | undefined {
    return this.sorbetLanguageClient;
  }

  private set activeSorbetLanguageClient(
    value: SorbetLanguageClient | undefined,
  ) {
    if (this.sorbetLanguageClient === value) {
      return;
    }

    // Clean-up existing client, if any.
    if (this.sorbetLanguageClient) {
      this.sorbetLanguageClient.dispose();
      const i = this.disposables.indexOf(this.sorbetLanguageClient);
      if (i !== -1) {
    ...
    this.sorbetLanguageClient = value;

    // State might have changed based on new client.
    if (this.sorbetLanguageClient) {
      this.onStatusChangedEmitter.fire({
        status: this.sorbetLanguageClient.status,
        error: this.sorbetLanguageClient.lastError,
      });
    }
  }
