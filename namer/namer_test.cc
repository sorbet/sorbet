...
public:
    void SetUp() {
        ctxPtr = make_unique<ast::ContextBase>(*console);
    }
    ast::Context getCtx() {
...
private:
    unique_ptr<ast::ContextBase> ctxPtr;
};
...
std::unique_ptr<ast::Statement> hello_world(ast::ContextBase &ctx) {
...
TEST_F(NamerFixture, HelloWorld) {
    ...
    auto objectScope = ast::ContextBase::defn_object().info(ctx);
    ASSERT_EQ(ast::ContextBase::defn_root(), objectScope.owner);
    ...
    ASSERT_EQ(ast::ContextBase::defn_object(), symbol.owner);
    ASSERT_EQ(0, symbol.arguments().size());
    ASSERT_EQ(ast::ContextBase::defn_todo(), symbol.result());
}
...
TEST_F(NamerFixture, NameClass) {
    ...
    namer::Namer::run(ctx, std::move(tree));
    auto rootScope = ast::ContextBase::defn_root().info(ctx);
