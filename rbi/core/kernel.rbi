...
module Kernel
  ...
  # converted using *arg* .to\_f. Converting a `string` with invalid
  ...
  # [Numeric](https://ruby-doc.org/core-2.6.3/Numeric.html) types are
  # converted directly (with floating point numbers being truncated). *base*
  # (0, or between 2 and 36) is a base for integer string representation. If
  # *arg* is a `String`, when *base* is omitted or equals zero, radix
  # indicators ( `0`, `0b`, and `0x` ) are honored. In any case, strings
  ...
  # `Kernel.exit(false)` . If *msg* is given, it is written to STDERR prior
  # to terminating.
  sig do
    params(
        msg: String,
  ...
  # contents of each record. A separator of `nil` reads the entire contents,
  # and a zero-length separator reads the input one paragraph at a time,
  # where paragraphs are divided by two consecutive newlines. If the first
  # argument is an integer, or optional second argument is given, the
  ...
  # in the library directories listed in `$:` . If the optional *wrap*
  ...
  # no circumstance will any local variables in the loaded file be
  ...
  # Equivalent to `Kernel::gets`, except `readline` raises `EOFError` at
  # end of file.
  sig do
    params(
        arg0: String,
  ...
  # [LoadError](https://ruby-doc.org/core-2.6.3/LoadError.html) is raised.
  ...
  # array that contains arrays of those
  ...
  # number, including a `Float` with fractional seconds). Returns the actual
  ...
  # Transfers control to the end of the active `catch` block waiting for
  # *tag* . Raises `UncaughtThrowError` if there is no `catch` block for the
  # *tag* . The optional second parameter supplies a return value for the
  # `catch` block, which otherwise defaults to `nil` . For examples, see
  # `Kernel::catch` .
  sig do
    params(
        tag: Object,
