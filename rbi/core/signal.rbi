...
module Signal
  ...
  # The characters “SIG” may be omitted from the signal name. The command or
  # block specifies code to be run when the signal is raised. If the command
  # is the string “IGNORE” or “SIG\_IGN”, the signal will be ignored. If the
  # command is “DEFAULT” or “SIG\_DFL”, the Ruby’s default handler will be
  # invoked. If the command is “EXIT”, the script will be terminated by the
  # signal. If the command is “SYSTEM\_DEFAULT”, the operating system’s
  # default handler will be invoked. Otherwise, the given command or block
  # will be run. The special signal name “EXIT” or signal number zero will
  # be invoked just prior to program termination. trap returns the previous
