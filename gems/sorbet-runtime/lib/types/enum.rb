...
class T::Enum
  ...
  def serialize
    # assert_bound!, inlined to avoid the extra frame on this hot path
    if @const_name.nil?
  ...
  sig { returns(String).checked(:never) }
  def inspect
    @inspect_str || "#<#{self.class.name}::#{@const_name || '__UNINITIALIZED__'}>"
  end
  ...
  def initialize(serialized_val=UNSET)
    ...
    @inspect_str = T.let(nil, T.nilable(String))
  ...
  def _bind_name(const_name)
    ...
    # Memoize inspect before the freeze below, but only for named classes:
    # anonymous classes (name still nil here) keep the dynamic interpolation
    # in #inspect so a later name assignment is reflected.
    class_name = self.class.name
    @inspect_str = "#<#{class_name}::#{const_name}>".freeze if class_name
    freeze
  end
