...
module T::Private::Methods::SignatureValidation
  ...
  # Same evaluation order and side effects as the inline `all?` it replaces:
  # check_tests? must only run for a :tests-checked sig (it side-effectfully
  # arms the @wrapped_tests_with_validation trapdoor in RuntimeLevels).
  ...
  def self.validate_override_types(signature, super_signature)
    ...
    #
    # Index loop instead of zip+each_with_index (saves the zip array plus one
    # pair array per positional arg). Iterating to super's length replicates
    # zip exactly: truncation when the override has more named args, and nil
    # padding (nil name/type, still checked) when the override has a rest
    # param and fewer named args than the base.
    super_arg_types = super_signature.arg_types
    arg_types = signature.arg_types
    index = 0
    while index < super_arg_types.length
      super_type = super_arg_types[index][1]
