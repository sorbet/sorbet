...
module T::Private::Methods
  ...
  def self.finalize_proc(decl)
    ...
    if decl.mod != PROC_TYPE
      raise "You are passing a Builder as a type. Did you accidentally use `self` inside a `sig` block?"
    end
  ...
  def self.run_builder(declaration_block)
    builder = Builder.new(declaration_block.mod)
    builder.
