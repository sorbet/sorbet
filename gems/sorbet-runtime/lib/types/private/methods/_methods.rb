...
module T::Private::Methods
  ...
  def self._on_method_added(hook_mod, mod, method_name)
    ...
    if current_declaration.nil?
      ...
      # When a class has multiple ancestors with `method_added` hooks (via
      # T::Sig), a single `def` fires `method_added` multiple times (once per
      # ancestor, chained via `super`). The first firing consumes the
      # declaration and registers the sig_wrapper. Subsequent firings see
      # `current_declaration == nil`. We must not delete the sig_wrapper in
      # that case — only for genuine redefinitions (a new `def` without a sig).
      #
      # We detect the duplicate by checking whether this key was the last one
      # registered on this DeclState (set by the method_added hook when we
      # return a non-nil key). If it matches, this is a duplicate notification
      # from `super` chaining, not a genuine redefinition.
