...
module T::Props
  module Private
    class ApplyDefault
      ...
      sig { params(accessor_key: Symbol, setter_proc: SetterFactory::SetterProc, bound_setter_proc: SetterFactory::BoundSetterProc).void.checked(:never) }
      def initialize(accessor_key, setter_proc, bound_setter_proc)
      ...
      def self.for(cls, rules)
        ...
        if rules.key?(:factory)
          ApplyDefaultFactory.new(cls, rules.fetch(:factory), accessor_key, setter, bound_setter)
        elsif rules.key?(:default)
          default = rules.fetch(:default)
          case default
          when *NO_CLONE_TYPES
            return ApplyPrimitiveDefault.new(default, accessor_key, setter, bound_setter)
          when String
            if default.frozen?
              return ApplyPrimitiveDefault.new(default, accessor_key, setter, bound_setter)
            end
          when Array
            if default.empty? && default.class == Array
              return ApplyEmptyArrayDefault.new(accessor_key, setter, bound_setter)
            end
          when Hash
            if default.empty? && default.default.nil? && T.unsafe(default).default_proc.nil? && default.class == Hash
              return ApplyEmptyHashDefault.new(accessor_key, setter, bound_setter)
            end
          end

          ApplyComplexDefault.new(default, accessor_key, setter, bound_setter)
        else
          nil
        end
      end
    ...
    class ApplyFixedDefault < ApplyDefault
      ...
      # checked(:never) - We do this with `T.let` instead
      sig { params(default: BasicObject, accessor_key: Symbol, setter_proc: SetterFactory::SetterProc, bound_setter_proc: SetterFactory::BoundSetterProc).void.checked(:never) }
      def initialize(default, accessor_key, setter_proc, bound_setter_proc)
        ...
        super(accessor_key, setter_proc, bound_setter_proc)
      end
    ...
    class ApplyDefaultFactory < ApplyDefault
      ...
      def initialize(cls, factory, accessor_key, setter_proc, bound_setter_proc)
