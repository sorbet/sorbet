...
SimilarMethodsByName similarMethodsForClass(const core::GlobalState &gs, core::ClassOrModuleRef receiver,
    ...
    for (auto ancestor : ancestors(gs, receiver)) {
        ...
        for (auto [memberName, memberSymbol] : ancestor.data(gs)->members()) {
            ...
            if (hasAngleBrackets(memberName.shortName(gs)) && memberName != core::Names::caseAngles()) {
...
core::ClassOrModuleRef unwrapSubclassToSymbol(const core::TypePtr &subclass) {
    if (auto right = core::cast_type<core::AppliedType>(subclass)) {
        return right->klass;
    } else if (core::isa_type<core::ClassType>(subclass)) {
...
std::unique_ptr<CompletionItem> CompletionTask::getCompletionItemForCase(const core::GlobalState &gs,
    ...
    if (!iter->isBottom()) {
        ...
        while ((orT = core::cast_type<core::OrType>(*iter))) {
            iter = &orT->left;
            values.emplace_back(unwrapSubclassToSymbol(orT->right));
        }
        // Last thing is not an OrType, because the final union with `T.noreturn` collapses
        values.emplace_back(unwrapSubclassToSymbol(*iter));
    } else {
        // An enum with no values is possible, but it's not clear what we should do.
        //
        // Adding this conditional makes Sorbet generate a (syntactically invalid) `case/else`.
        // Another option would be to skip suggestion a completion item altogether, but that might leave
        // people wondering why there isn't an item when they're expecting--at least this way gives an
        // indication to the user that the enum is empty, which is something to address
    }

    // Sometimes the receiverLoc doesn't exist, e.g. for `x&.case` calls
    // If it doesn't exist, we default to what it would be if it's empty (self) mostly out of convenience,
    // not because it's correct.
    auto receiverLoc = core::Loc(queryLoc.file(), params.receiverLoc);
    auto receiverText = receiverLoc.exists() && !receiverLoc.empty() ? receiverLoc.source(gs).value() : "self";
    ...
    item->insertText = to_string(buf);

    if (receiverLoc.exists() && !receiverLoc.empty()) {
        // For a plethora of reasons, it's easier if we use `insertText` + `additionalEdits`,
        // instead of trying to set `textEdit`, due to idiosyncrasies discovered while testing
        // in VS Code:
        //
        // - `textEdit` doesn't seem to play well with multi-line replacement text
        // - `textEdit` doesn't seem to be able to remove a prefix that crosses a `.` (e.g. `my_enum.ca`)
        // - `insertText` automatically gets the "adjust indentation" insertion mode
        // - Sorbet sets `isIncomplete=false`, so VS Code doesn't send further requests. By
        //   avoiding `textEdit`, the only range we send will always be valid, even as the user keeps typing.
        item->additionalTextEdits = vector<unique_ptr<TextEdit>>{};
        item->additionalTextEdits->emplace_back(
            make_unique<TextEdit>(Range::fromLoc(gs, core::Loc(queryLoc.file(), params.receiverLoc.beginPos(),
                                                               queryLoc.beginPos() - prefix.size())),
                                  ""));
    }
    return item;
}
