...
void LSPIndexer::transferInitializeState(InitializedTask &task) {
    ...
    auto typecheckerGS = std::exchange(
        this->gs,
        this->gs->copyForIndex(this->config->opts.extraPackageFilesDirectoryUnderscorePrefixes,
                               ...
                               this->config->opts.stripePackagesHint, this->config->opts.genPackages, this->config->opts.genPackagesStrict));
