...
void generateErrors(core::FileRef fref, vector<unique_ptr<core::Error>> &errors, uint16_t count) {
    for (uint16_t i = 0; i < count; i++) {
        errors.emplace_back(make_unique<core::Error>(
            core::Loc(fref, 0, 0), core::ErrorClass{1, core::StrictLevel::True}, "MyError",
            vector<core::ErrorSection>(), vector<core::AutocorrectSuggestion>(), false, core::ErrorSeverity::Error));
    }
}
