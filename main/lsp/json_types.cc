...
core::Loc Range::toLoc(const core::GlobalState &gs, core::FileRef file) const {
    ...
    auto sPos = core::Loc::pos2Offset(file.data(gs), core::Loc::Detail{(uint32_t)start->line + 1, (uint32_t)start->character + 1});
    auto ePos = core::Loc::pos2Offset(file.data(gs), core::Loc::Detail{(uint32_t)end->line + 1, (uint32_t)end->character + 1});
