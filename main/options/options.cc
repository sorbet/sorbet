...
buildOptions(const vector<pipeline::semantic_extension::SemanticExtensionProvider *> &semanticExtensionProviders) {
    ...
    options.add_options("advanced")("error-kwarg-hash-without-splat",
                                    "Report old-style (pre-Ruby 3.0) keyword argument hash without '**' as an "
                                    "error",
                                    cxxopts::value<bool>());
...
void readOptions(Options &opts,
    ...
    try {
        ...
        opts.errorKwargHashWithoutSplat = raw["error-kwarg-hash-without-splat"].as<bool>();
