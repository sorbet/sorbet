...
buildOptions(const vector<pipeline::semantic_extension::SemanticExtensionProvider *> &semanticExtensionProviders) {
    ...
    options.add_options(section)(
        ...
        "File budget for the fast path: an edit that changes more than <count> files, or whose changes would require "
        "re-typechecking more than <count> files, takes the slow path (a full re-typecheck of the workspace). Edits "
        "the fast path cannot handle at all (new files, package files, syntax errors, changed definitions) take the "
        "slow path regardless of <count>. Tools that regenerate many files at once can make a higher <count> "
        "worthwhile.",
