...
namespace {
core::FileHash computeFileHash(const core::GlobalState &gs, ast::ParsedFile parsedFile) {
    Timer timeit(gs.tracer(), "computeFileHash");
    const static options::Options emptyOpts{};
    unique_ptr<core::GlobalState> lgs = make_unique<core::GlobalState>(
        (make_shared<core::ErrorQueue>(gs.tracer(), gs.tracer(), make_shared<core::NullFlusher>())));
    lgs->initEmpty();
    ...
    core::UsageHash hash;
    {
        ...
        hash = substitution.getAllNames();
    }

    vector<ast::ParsedFile> single;
    single.emplace_back(move(parsedFile));

    auto workers = WorkerPool::create(0, lgs->tracer());
    pipeline::resolve(lgs, move(single), emptyOpts, *workers, true);

    return {move(*lgs->hash()), move(hash)};
}
}; // namespace

void computeFileHashes(core::GlobalState &gs, const vector<ast::ParsedFile> &trees, WorkerPool &workers) {
    Timer timeit(gs.tracer(), "computeFileHashes");
    shared_ptr<ConcurrentBoundedQueue<int>> fileq = make_shared<ConcurrentBoundedQueue<int>>(trees.size());
    for (int i = 0; i < trees.size(); i++) {
        auto copy = i;
        fileq->push(move(copy), 1);
    }

    gs.tracer().debug("Computing state hashes for {} files", trees.size());

    shared_ptr<BlockingBoundedQueue<vector<pair<int, unique_ptr<const core::FileHash>>>>> resultq =
        make_shared<BlockingBoundedQueue<vector<pair<int, unique_ptr<const core::FileHash>>>>>(trees.size());
    workers.multiplexJob("lspStateHash", [fileq, resultq, &trees, &gs]() {
        ...
        int job;
        {
            for (auto result = fileq->try_pop(job); !result.done(); result = fileq->try_pop(job)) {
                if (result.gotItem()) {
                    ...
                    if (!tree.tree || tree.file.data(gs).getFileHash() != nullptr) {
                        continue;
                    }

                    threadResult.emplace_back(job, make_unique<core::FileHash>(computeFileHash(
                                                       gs, ast::ParsedFile{tree.tree.deepCopy(), tree.file})));
                }
            }
        }
        ...
        for (auto result = resultq->wait_pop_timed(threadResult, WorkerPool::BLOCK_INTERVAL(), gs.tracer());
            ...
            if (result.gotItem()) {
                for (auto &a : threadResult) {
                    trees[a.first].file.data(gs).setFileHash(move(a.second));
                }
            }
        }
    }
