    ...
    switch (file.dataAllowingUnsafe(gs).sourceType) {
        case core::File::Type::NotYetRead: {
...

            {
                ...
                fileObj->setIsMissing(fileMissing);

                if constexpr (enable_counters) {
                    counterAdd("types.input.lines", fileObj->lineCount());
                }
...
struct IndexResult {
    ...
    // we risk potential deadlock with bounded queues that don't appear to have the right number of items present.
...

    {
        ...
        workers.multiplexJob("substituteTrees", [&cgs, batchq, resultq, cancelable]() {
            ...
            for (auto result = batchq->try_pop(job); !result.done(); result = batchq->try_pop(job)) {
                if (result.gotItem()) {
                    // Unconditionally update the total to avoid deadlocking the consumer
